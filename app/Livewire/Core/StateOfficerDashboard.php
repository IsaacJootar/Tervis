...
class StateOfficerDashboard extends Component
{
  ...
  public function mount()
  {
    ...
    if (!$user || !in_array($user->role, ['State Data Administrator', 'State Administrator'])) {
      abort(403, 'Unauthorized: Only State Administrators can access this page.');
    }

    // Get state-wide scope
    $scope = $this->scopeService->getUserScope();

    if (empty($scope['facility_ids']) || !isset($scope['facility_ids'])) {
      Log::warning('No facilities found for state scope', ['user_id' => $user->id, 'state_id' => $user->state_id]);
      $this->facilityIds = [];
      $this->facilityCount = 0;
      $this->facilities = collect([]);
      $this->state_id = $user->state_id;
      $this->state_name = State::find($user->state_id)->name ?? 'N/A';

      toastr()->warning('No facilities found for your state scope.');
      return;
    }
    ...
    Log::info('State Dashboard Initialized', [
      'user_id' => $user->id,
  ...
  public function loadDashboardData()
  {
    ...
    try {
      ...
      Log::error('State Dashboard data loading failed: ' . $e->getMessage(), [
        'state_id' => $this->state_id,
  ...
  private function getFilteredTrendData($startDate)
  {
    try {
      ...
      Log::error('State Trend data calculation failed: ' . $e->getMessage());
      return [
        'labels' => [],
        'antenatal' => [],
        'delivery' => [],
  ...
  private function getOverviewStats($startDate)
  {
    try {
      ...
      Log::error('State Overview stats calculation failed: ' . $e->getMessage());
  ...
  private function getRegisterStats($startDate)
  {
    try {
      ...
      Log::error('State Register stats calculation failed: ' . $e->getMessage());
      return [
  ...
  private function getDemographicData()
  {
    try {
      ...
      Log::error('State Demographic data calculation failed: ' . $e->getMessage());
      return ['age_groups' => []];
    }
  }
  ...
  private function getPerformanceMetrics($startDate)
  {
    try {
      ...
      Log::error('State Performance metrics calculation failed: ' . $e->getMessage());
      return [
  ...
  private function getRiskAlerts()
  {
    ...
    try {
      ...
      if ($highRiskCount > 0) {
        $alerts[] = [
          'type' => 'warning',
          'title' => 'High-Risk Pregnancies (State-wide)',
      ...
      if ($overdueCount > 0) {
        $alerts[] = [
          'type' => 'danger',
          'title' => 'Overdue Deliveries (State-wide)',
          'message' => "{$overdueCount} patients past expected delivery date across state",
          'count' => $overdueCount,
          'icon' => 'bx-calendar-x'
        ];
      }
      ...
      if ($overdueVaccinations > 0) {
        $alerts[] = [
          'type' => 'warning',
          'title' => 'Overdue TT Vaccinations (State-wide)',
      ...
      if ($inactivePatients > 0) {
        $alerts[] = [
          'type' => 'warning',
          'title' => 'Inactive Patients (State-wide)',
      ...
      Log::error('State Risk alerts calculation failed: ' . $e->getMessage());
    }

    return $alerts;
  }

  private function getLgaBreakdown($startDate)
  {
    try {
      $breakdown = [];

      foreach ($this->facilities->groupBy('lga') as $lga => $lgaFacilities) {
        $lgaFacilityIds = $lgaFacilities->pluck('id')->toArray();
        ...
        $breakdown[$lga] = [
          ...
          'high_risk_cases' => $this->calculateHighRiskCasesForFacilities($lgaFacilityIds),
        ];
      }
      ...
      Log::error('LGA breakdown calculation failed: ' . $e->getMessage());
      return [];
    }
  }
  ...
  private function calculateTrend($register, $startDate)
  {
    try {
      ...
      Log::error('State Trend calculation failed: ' . $e->getMessage());
      return 0;
    }
  }
  ...
  private function calculateHighRiskCases()
  {
    try {
      ...
      Log::error('State High-risk cases calculation failed: ' . $e->getMessage());
      return 0;
    }
  }
  ...
  {
    return view('livewire.core.state-officer-dashboard')
      ->layout('layouts.stateOfficerLayout');
  }
}
